...
mygraph = rcx_toNGraph(mynetwork)
...
Afterwards `ndex_find_networks` retrieves a list of networks of user `nci-pid`, which contains the data of the Pathway Interaction Database. The function `ndex_get_network` downloads the network data and stores in the `RCX` format (explained in the next section) and is then converted into an igraph-based object via `rcx_toNGraph`.
...
“Version\_3.0”) and update the R code in `/R/ndex_api_config.r`, which defines the `ndex_config` object.
```{r, eval=FALSE}
