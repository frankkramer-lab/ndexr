...
networks_pid <- ndex.find.networks(ndexcon, accountName="nci-pid")
...
mynetwork = ndex.get.network(ndexcon, networks_pid[1,"externalId"])

## convert into R graph format
mygraph = rcx.toNGraph(mynetwork)
...
Afterwards `ndex.find.networks` retrieves a list of networks of user `nci-pid`, which contains the data of the Pathway Interaction Database. The function `ndex.get.network` downloads the network data and stores in the `RCX` format (explained in the next section) and is then converted into an igraph-based object via `rcx.toNGraph`.
